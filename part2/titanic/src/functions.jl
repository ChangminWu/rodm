...
function createFeatures(dataFolder::String, dataSet::String)
    ...
    if !isfile(trainDataPath) || !isfile(testDataPath)
        ...
        if dataSet == "titanic"
            # Binaize Survived, Sex
            features[:Survived] = ifelse.((rawData[:Survived].==1), 1, 0)
            features[:Sex] = ifelse.((rawData[:Sex].=="male"), 1, 0)

            # Binaize Pclass, Age, Siblings Abroad, Parents abroad, Fare
            createColumns(:Pclass, [0,20,40,60,80,Inf], rawData, features)
            createColumns(:Age, [1,2,3,Inf], rawData, features)
            createColumns(Symbol("Siblings/Spouses Aboard"), [0,1,2,3,5,Inf], rawData, features)
            createColumns(Symbol("Parents/Children Aboard"), [0,1,2,3,Inf], rawData, features)
            createColumns(:Fare, [0,50,100,200,300,Inf], rawData, features)
...
function createRules(dataSet::String, resultsFolder::String, train::DataFrames.DataFrame)
    ...
    if !isfile(rulesPath)
        ...
        for y = 0:1
            ...
            # model
            m = Model(solver = CplexSolver(CPX_PARAM_TILIM=600, CPX_PARAM_SCRIND=0))
            @variable(m, x[i in 1:n], Bin)
            @variable(m, b[i in 1:d], Bin)

            @constraint(m, [i=1:n, j=1:d], x[i]<=1+(t[i,j]-1)*b[j])
            @constraint(m, [i=1:n], x[i]>=1+sum((t[i,j]-1)*b[j] for j in 1:d))
            @constraint(m, sbarx, sum(x[i] for i in 1:n) <= sx )
            @objective(m, Max, sum(x[i] for i in 1:n if transactionClass[i,1]==y)-RgenX*sum(x[i] for i in 1:n)-RgenB*sum(b[j] for j in 1:d))

            while sx > n * mincovy
                JuMP.setRHS(sbarx, sx)
                if iter == 1
                    solve(m)
                    s = sum(round(Int, getvalue(x[i])) for i in 1:n if transactionClass[i,1]==y)
                    iter += 1
                    rule = convert(DataFrame, hcat(append!([y], round.(Int, getvalue(b)))...))
                    rules = append!(rules,rule)
                    @constraint(m, sum(b[j] for j in 1:d if round(Int, getvalue(b[j]))==0)+sum(1-b[j] for j in 1:d if round(Int, getvalue(b[j]))==1)>=1)
                elseif iter < iterlim
                    solve(m)
                    snew = sum(round(Int, getvalue(x[i])) for i in 1:n if transactionClass[i,1]==y)
                    if snew < s
                        sx = min(sx-1, sum(round.(Int, getvalue(x))))
                        iter = 1
                    else
                        iter += 1
                    end
...
function sortRules(dataSet::String, resultsFolder::String, train::DataFrames.DataFrame, rules::DataFrames.DataFrame)
    ...
    if !isfile(orderedRulesPath)
        ...
        #insert!.(rules.columns, 1, append!([0], zeros(d))) # return error
        #insert!.(rules.columns, 1, append!([1], zeros(d))) # return error
        push!(rules, 1, append!([0], zeros(d)))
        push!(rules, 1, append!([1], zeros(d)))
        rules = unique(rules)
